...
from utils import data_location, draw_landmarks_custom, extract_landmarks, mediapipe_detection
...
def collect_training_data():
    ...
    with mp.solutions.holistic.Holistic(min_detection_confidence=MIN_DETECTION_CONFIDENCE,
...
def draw_landmarks(image, results, gesture, sequence, frame_count):
    ...
    cv2.putText(image, f"Frames being collected for {gesture} video number {sequence}",
                (15, 12), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA)
    cv2.imshow("Feed", image)
    if frame_count == 0:
        cv2.putText(image, "The program will now start to collect training data",
